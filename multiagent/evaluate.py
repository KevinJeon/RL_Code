...
class ZeroShotAgents(object):
    def __init__(self, model_types, load_dirs, obs_sizes, num_actions, file_name, agent1, agent2):
        self.models = []
        ...
        for model, pth, obs_size, num_action in zip(model_types, self.load_dirs, obs_sizes, num_actions):
            if model == 'maddpg':
...
def parse_args():
    ...
    parser.add_argument('--base_dir', '-l', default=['./zeroshot', './zeroshot'], type=str)
    parser.add_argument('--file_name', '-f', default='.h5', type=str)
    parser.add_argument('--model_types', '-m', default=['maddpg', 'maddpg'], type=list)
...
def main(args):
    ...
    env_dir = [args.base_dir[0] + '/{}'.format(args.env_name)] * 2
    agents = os.listdir(args.base_dir[0]+'/{}'.format(args.env_name))
    jd = dict()
    for agent1 in sorted(agents):
        jd[agent1] = dict()
        for agent2 in sorted(agents):
            models = ZeroShotAgents(args.model_types, env_dir , obs_sizes, num_actions, args.file_name, agent1, agent2)
